...
require_once (dirname(__FILE__) . "/salt/autoload.php");
class Base32 {
...
class KeyPair {
    ...
    private function getPrefixFromId($networkId)
    {
        foreach ($this->networkType as $name => $spec) {
    ...
    private function calcAddress($networkId = 104) {
        $sha3Hash = Sha3::keccakhash($this->binaryPublic, 256, true);
        $ripemd160Hash = hash("ripemd160", $sha3Hash, true);
        $networkPrefix = $this->getPrefixFromId($networkId);
        $withNetworkByte = hex2bin($networkPrefix) . $ripemd160Hash;
    ...
    public function getAddress($networkId = 104) {
        if ($this->Failed) {
            return false;
        }
        return $this->calcAddress($networkId);
    }
