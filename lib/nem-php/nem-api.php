 ...
 * Time: 20:08
 */
require_once dirname(__FILE__) . '/TransactionBuilder.php';
require_once dirname(__FILE__) . '/NemUtil.php';
 ...
 */
     ...
     * @return array
     */
    public function transactionGet($hash=null, $decode=true){

        $trxGet = $this->fetch_nis('/transaction/get',
            [
                'hash' => $hash
            ]
        , 'GET');

        if (isset($trxGet['result']['transaction']['message']['payload']) && $decode) {
            $trxGet['result']['transaction']['message']['payload'] = hex2bin($trxGet['result']['transaction']['message']['payload']);
        }
        return $trxGet;
    }
     ...
     * @return array
     */
    public function requestAnnounce(){

        $kp = new KeyPair($this->private);
        $tx = new TransactionBuilder($this->transaction);

        // Sign the transaction.
        $signature = $kp->sign($tx->getBinary());

        return $this->fetch_nis('/transaction/announce',
            [
                'data'          => $tx->getHex(),
                'signature'     => $signature
            ]
